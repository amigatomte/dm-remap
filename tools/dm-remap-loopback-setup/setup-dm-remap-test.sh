...
DESCRIPTION:
  ...
  Can also add bad sectors on-the-fly to an existing dm-linear device.
...
MODES:
  ...
  Add Bad Sectors Mode:
    --add-bad-sectors       Add bad sectors on-the-fly to existing dm-linear device
                            (requires existing dm-test-linear device)
...
OPTIONS:
  ...
  Add Bad Sectors Mode Options:
    Same -f, -c, -p options to specify which sectors to add
...
EXAMPLES:
  ...
  # Add 50 more random bad sectors on-the-fly to existing device
  sudo ./setup-dm-remap-test.sh --add-bad-sectors -c 50 -v
  
  # Add specific bad sectors from file on-the-fly
  sudo ./setup-dm-remap-test.sh --add-bad-sectors -f additional_bad_sectors.txt -v
  
  # Add 10% more bad sectors on-the-fly
...
# Extract current bad sectors from dm-linear table
extract_current_bad_sectors() {
    ...
    # Parse table for error entries: "START SIZE error"
    while read -r start size target; do
        if [[ "$target" == "error" ]]; then
            # Add all sectors in this error range
...
# Update dm-linear table with new bad sectors on-the-fly
update_linear_table_on_fly() {
    ...
    # Get current size
    local total_size=$(dmsetup table "$DM_LINEAR_NAME" 2>/dev/null | awk '{print $1+$2}' | sort -n | tail -1)
    ...
    if (( DRY_RUN )); then
        log_debug "DRY RUN: Would update dm-linear with $(echo "$table" | wc -l) table entries"
        return
    fi
    
    # Safely update the table:
    # 1. Suspend I/O to the device
...
handle_add_bad_sectors_mode() {
    ...
    # Check if dm-linear device exists
    ...
    # Generate new bad sectors to add
    local total_size=$(dmsetup table "$DM_LINEAR_NAME" 2>/dev/null | awk '{print $1+$2}' | sort -n | tail -1)
    local total_sectors=$total_size
    ...
    # Find the loopback device backing dm-linear
    local loop_device=$(dmsetup table "$DM_LINEAR_NAME" 2>/dev/null | grep "linear" | head -1 | awk '{print $4}')
    
    if [[ -z "$loop_device" ]]; then
        # If no linear entries, the table is all error entries
        log_warn "No linear entries found in current table, getting loop device from system..."
...
MONITORING:
  ...
  # View updated table
  dmsetup table $DM_LINEAR_NAME | head -20
